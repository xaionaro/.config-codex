...
- Without an active ECI/ATE root, explicit user workflow requests precede inference: `ECI` alone selects ECI; `ATE` alone or both select ATE. Start ATE only when the user explicitly asks to use ATE or `agent-teams-execution`. Descriptive mentions and requests to inspect or edit its skill documents do not invoke ATE.
