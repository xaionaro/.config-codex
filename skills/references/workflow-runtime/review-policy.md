...
Normal reviewer packets contain original user requirements, exact target/diff, `loop-id`, applicable `decision-id`, objective/criteria, general pre-routing record, admitted style record/deltas/tool evidence, full applicable lineage/binding, and all scrutiny rules. For a checkpointed review, the packet names the checkpoint and states explicit exclusions. The checkpointed `current diff` is the named parent-to-checkpoint range. Respect explicit exclusions and exclude later ambient worktree changes. This is review scope, not admission proof. Include `started`, deadline, and `sealed-at` only for a potential defer. Every finding attaches to a specific option/target with severity, impact (`trivial` or `substantive`), one-line rationale, evidence location, and concrete fix direction. A missing impact tag is a defect in the review output; re-prompt or respawn it before gate evaluation.
