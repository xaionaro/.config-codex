...
`LOOP DETECTED` text emitted by a stop hook is control metadata, never a new user request. The hook remains an authoritative, fast `decision:block` while the active marker exists; it does not parse event keys, persist recovery state, or bypass the block. Keep loop-recovery state in memory/session-ledger state: derive a stable `blocker_id` from workflow, root, and unmet criterion, and a `state_fingerprint` from normalized cause, capability/schema/profile state, completed outcomes, owner, and exact unblock. Exclude timestamps, event IDs, wording, timer counts, and status formatting. Record each distinct recovery action and outcome once. A repeated fingerprint is a no-op: do not repeat an action, wait, retry, final/status reply, or user question. Resume only after materially changed content—completed distinct action, new source/test evidence, changed tool/schema/profile/resource, or concrete user input; a new key alone does not resume. Enter recovery once, perform one distinct recovery action, then enter `awaiting-event`. Resume once only for a genuinely new direct user turn or provider-delivered event; duplicate/missing keys, repeated callbacks, timers, status messages, and timeouts are no-ops. Continue normal ATE, nested ECI, and BRP while a feasible internal path exists. Do not bias toward escalation. Quiet only after BRP proves no feasible internal path and the exact missing input, resource, or decision is concrete user-owned and unobtainable; emit one blocker report/question. Provider or tool ownership alone never qualifies. A running expected agent completion remains allowed. Do not emit another final/status reply for the same event. `awaiting-event` belongs to the coordinator and is not a hook-marker phase. Nested ECI is owned by outer ATE; normal teardown rules remain unchanged.
...
If the current collaboration schema exposes no effective telemetry, record `effective-unavailable` and admit the requested special role after all exposed selectors are accepted. Missing or rejected exposed selectors and conflicting returned effective telemetry reject only that child dependency; continue ATE/nested ECI and BRP without ordinary fallback, reuse, or downgrade.
...
- [ ] Special selector gate: send every exposed `sol-high` selector (currently `model="gpt-5.6-sol"` and `reasoning_effort="high"`); record unavailable dimensions, including provider binding when not exposed, as `unavailable_by_schema`; reject omitted or rejected selectors only for that child
- [ ] Special invocation gate: record `requested-special` plus child identity after a non-rejecting spawn, or `effective-unavailable` when effective telemetry is unavailable; no provider receipt, sidecar, self-attestation, or invented artifact is required; reject conflicting returned effective telemetry only for that child and never use ordinary fallback, reuse, or downgrade
...
- [ ] For execution review spawns: reusable ordinary correctness/fidelity slot assigned and a fresh special long-term-health spawn with unique transport/boundary/profile evidence plus requested-special/effective-unavailable record; retire the prior special slot without shutdown or terminal cleanup.
...
- `special_ordinary_model_fallback`: send every exposed selector for `sol-high`; record unavailable-by-schema dimensions as `effective-unavailable`, not `special_ordinary_model_fallback`; reject malformed, unsupported, omitted, or rejected exposed selectors and conflicting effective telemetry only for that child, never ordinary fallback, reuse, or downgrade.
...
- `special_prompt_hash_rationalization`: prompt text, hash, parent defaults, successful spawn, and roster/ledger records prove requested values and child identity only; they never prove effective application or justify an invented receipt/sidecar/self-attestation.

### Focused policy-pressure scenarios

Apply these scenarios within the nine counters above:

- `requested-special/effective-unavailable`: exposed selectors are accepted; record `requested-special`, child identity, and `effective-unavailable`; admit the special role without receipt/sidecar.
...
- `keyless-new-evidence`: new source/test evidence with no new event key changes the fingerprint and permits one new action.
