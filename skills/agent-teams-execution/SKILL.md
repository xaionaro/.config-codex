...
After BRP proves no feasible internal path and identifies concrete unobtainable user-owned input/resource/decision, the coordinator may create the canonical user-owned wait report and run `CODEX_ROLE=coordinator ~/.codex/bin/eci-active wait <report.md>`. The report is exactly eight LF-terminated lines: `# ECI User-Owned Wait`, `state: user-owned-wait`, a safe `blocker_id`, a lowercase 64-hex `state_fingerprint`, `owner: user`, `brp_result: exhausted-no-feasible-internal-path`, `user_owned_input: unobtainable`, and a concrete `unblock` naming the missing user-owned input/resource/decision. This writes one validated `eci_wait` state while retaining the `eci_active` marker; the next stop consumes that state once and returns `continue`. It is coordinator recovery state, not persisted hook loop state, a generic escape hatch, or teardown. On materially changed normalized content, the coordinator must run `CODEX_ROLE=coordinator ~/.codex/bin/eci-active resume <new-state-fingerprint>`; the same fingerprint is rejected. Invalid or missing wait state keeps the stop blocked, and normal `off`/clean-pass remains required for teardown.
