...
**Aggregate implementation.** Research and design are global. After design, executors finish all root-task slices and discovered sub-tasks before code review. Unit tests stay with code. Root-task E2E/targeted proof gates review; failures spawn tasks. Review runs on the integrated diff: REJECTED findings rerun review, CONDITIONAL fixes apply before final proof. Keep one root-task commit per touched repo when feasible; amend/squash review fixes into it.

| Stage | Scope | When it starts |
|-------|-------|---------------|
| Research | Global | Immediately |
| Design | Global | After research |
| Execution | Per task/slice | After design approved. No code review yet. |
| E2E confirmation | Root task | After all known slices/sub-tasks land. Failures spawn tasks. |
| Aggregate review | Root task | After E2E proves the root task is fulfilled. Loop fixes until approved. |
| Post-review E2E + QA | Final | After aggregate review approval. |
...
| **Execution Reviewer** | 2 per root task | 3 | Independent aggregate review after root-task E2E confirmation: correctness/fidelity + long-term health. Report only, never edit code. |
...
One execution lane per independent unit. Two execution reviewers per root aggregate, spawned for aggregate review. Above ~4 active agents per phase, ensure strictly independent work.
...
| submitted → in_review | Coordinator verifies submission checklist passes. Routes full root-task diff to both execution reviewers; other work to paired reviewer |
| in_review → in_progress | Any reviewer REJECTED. Create/fix tasks without review until root-task E2E passes again |
| in_review → in_testing | Review approved. Rerun full root-task E2E before completion/QA |
...
| Review | Two aggregate reviewers inspect the integrated diff. |
| Repair | Fix findings, rerun root proof, amend/squash, rerun review on REJECTED. |
| Final proof | After review approval, rerun full E2E/proof before QA. |
...
- Before aggregate review, run full root-task E2E/targeted proof. Failures become tasks. No review until proof passes.
- After aggregate review approval, rerun full root-task E2E before QA.
...
| Execution Reviewers | Executor | Aggregate code/test issue | Direct during aggregate review |
...
- Each candidate fix CCed to reviewers for **async advice only**. Reviewers cannot reject. Executor does not wait for replies. Advice is incorporated if useful, otherwise ignored.
...
- After proof passes, task → `submitted` → `in_review`. Reviewers critique the full aggregate diff+rationale, reject mitigation, and improve cleanup, hardening, and semantic correctness. Only stage where REJECTED counts.
...
**Execution dual review:** Two reviewers per root aggregate: correctness/fidelity and long-term health. Review independently first. Execution verdicts override Reviewer Protocol item 4: Critical or Foundational (approach/architecture/semantic-integrity flaw) -> REJECTED; Major -> CONDITIONAL (required fix, no re-review by itself); Minor -> NIT (optional). Each pass reviews the aggregate as a whole; only REJECTED loops.
...
**AGGREGATE REVIEW INVARIANT:** No per-sub-task code review. Start root-task review only after all known slices/sub-tasks land, root-task E2E/targeted proof passes, and one commit per touched repo exists. Review rejections become tasks; fix them, re-prove E2E, amend/squash, then rerun aggregate review. After approval, rerun full E2E before QA.
...
7. **Handle root "submitted" tasks.** Verify grouped commit(s), claim tags, critique log, RCA when applicable, and root-task E2E/proof. Bounce if incomplete. If complete, route the full diff to both execution reviewers; non-code work goes to paired verifier.
8. **Drive aggregate pipelines.** Keep creating/fixing discovered tasks until root E2E passes. Then aggregate review loops until approved, amend/squash commits, rerun full E2E, then spawn QA. Record checkpoint per root task: output, reviewers, evidence, git SHA.
...
13. **Enforce aggregate invariant.** No review before root E2E/proof. No QA before aggregate review approval + post-review E2E.
...
| Coordinator assigns new task to executor | Verify file ownership, dependencies, and root-task grouping |
...
- [ ] For aggregate review spawns: both execution reviewers confirmed alive before review starts
...
- [ ] Execution reviewer spawns include: lens (correctness/fidelity or long-term health), explicit "Load the `<language>-coding-style` skill via skill instructions" instruction + shared concerns register
...
[For aggregate reviewers:] Paired with [OTHER REVIEWER NAME]. Aggregate review requires correctness/fidelity + long-term health reviewers. Message executor directly.
...
| Per-sub-task code review requested | STOP. Aggregate changes under the root task; review after root-task E2E/proof |
...
| Reviewer feedback ignored | Coordinator enforces required fixes/follow-ups; re-review only on REJECTED |
...
| Only one execution reviewer spawned for aggregate review | Spawn both: correctness/fidelity + long-term health |
