...
- [ ] Claim scope — compare mechanism/predicate, emitted wording, supported wording, and boundary counterexample/negative test; deterministic checks are not described as LLM reviewers/classifiers; any LLM first-tool admission review is separate `PreToolUse` behavior configured through `CODEX_EDIT_PRE_REVIEWER`, with `LLM_EDIT_PRE_REVIEWER` and `CLAUDE_EDIT_PRE_REVIEWER` accepted only as lower-precedence compatibility aliases when earlier variables are unset.
