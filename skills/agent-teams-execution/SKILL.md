...
`LOOP DETECTED` text emitted by a stop hook is control metadata, never a new user request. The hook remains an authoritative, fast `decision:block` while the active marker exists; it does not parse event keys, persist recovery state, or bypass the block. The coordinator keeps loop-recovery state in memory/session-ledger state: enter recovery once, perform one distinct recovery action, then enter `awaiting-event`. Resume once only for a genuinely new direct user turn or provider-delivered event; duplicate/missing keys, repeated callbacks, timers, status messages, and timeouts are no-ops. Do not emit another final/status reply for the same event. `awaiting-event` belongs to the coordinator and is not a hook-marker phase. Nested ECI is owned by outer ATE; normal teardown rules remain unchanged.
...
For a special assignment, the positive provider-native route sends the resolved profile values through exposed collaboration fields: current `model="gpt-5.6-sol"`, `reasoning_effort="high"`, and `model_provider="openai"` (or future values read from `sol-high.config.toml`). The provider-native request receipt must return those exact request values together with the immutable profile snapshot; it is the only accepted application route. If `spawn_agent` rejects or does not expose either `model` or `reasoning_effort`, or returns no receipt, block before spawning. This uses only exposed request fields and makes no hidden effective-telemetry claim; post-spawn sidecar evidence remains mandatory for acceptance.

Before every spawn, branch on the category-role boundary artifact. For an ordinary assignment, resolve the current configured model, provider, and exact reasoning effort; the generic “omit model override unless the user explicitly requested one” rule applies only to ordinary assignments. Record exposed application fields or the unavailable-schema limitation; never invent an effective value. For a special assignment, the explicit pre-spawn gate resolves `sol-high`, its exact profile hash, provider/model/effort values, and a non-empty `application_route` descriptor, then rechecks the hash immediately before spawning; it does not require post-spawn `application_evidence`. The explicit post-spawn gate creates and verifies the immutable deterministic sidecar and its recorded path/SHA, requires sidecar profile path/hash and prompt-artifact hash to equal the pre-spawn snapshots, requires exact spawn/transport identity binding, and requires `observed_provider`, `observed_model`, and `observed_effort` each to equal the resolved profile snapshot plus the closed recomputed redacted `application_evidence` object; missing, malformed, changed, weak, or unverifiable evidence blocks acceptance and never falls back to ordinary routing. Do not reject a special assignment because `high` differs from the ordinary configured effort: special `high` is required and distinct.
...
- [ ] Special request receipt gate: pass exact resolved `model="gpt-5.6-sol"`, `reasoning_effort="high"`, and profile `model_provider="openai"` through exposed `spawn_agent` fields (future profile values are substituted); accept only the returned provider-native receipt plus immutable profile snapshot; reject missing/rejected fields or receipt
- [ ] Special post-spawn sidecar/application gate: after success, immutable non-overwriting `.special-model.json` path/SHA is verified; profile path/SHA, recomputed prompt-artifact SHA, exact spawn/transport identity, and observed provider/model/effort all equal the pre-spawn/resolved profile snapshot; closed recomputed redacted `application_evidence` object is verified before acceptance or routing; missing evidence blocks and special `high` is not rejected for differing ordinary effort
...
- [ ] For execution review spawns: reusable ordinary correctness/fidelity slot assigned and a fresh special long-term-health spawn with unique transport/boundary/profile/sidecar evidence; retire the prior special slot without shutdown or terminal cleanup.
