...
An ATE `ate_active` marker alone does not engage ECI's required-critic runtime gate. When an ATE route selects ECI, the coordinator must create the direct session marker with `~/.codex/bin/eci-active on "<task + scope>"` before governed work, keep it active, and use the ECI `commit`, `final`, and `off` boundaries. ATE remains the outer lifecycle owner; its marker is not a substitute for direct ECI engagement.
...
`skills/references/requirement-lineage.md`) before ATE performs coordinator
...
ATE-specific packet rules stay inline: task transitions, spawn checklists,
assignment packets, routing messages, reviewer prompts, review/E2E packets,
reusable-producer reassignments, and discovered-work proposals carry the shared
`Lane`, canonical executable `requirement_refs`; display-only `R<n>` requires the
bound alias map and is never a bare durable ref. They carry paths/edges,
register/state version,
`Assignment`/`admission_binding`, and expanded full chain. Critic C Packet 1
requires coordinator-side lineage admission, ledger/log pair verification,
exact prompt-artifact creation, and required provider/profile/identity checks
before spawning; these remain coordinator evidence, not Packet 1 body fields.
Only lineage serialization and normal review context are omitted from its body.
Packet 2 carries the full register and graph evidence. ATE owns canonical
status and projections for nested ECI.
...
Before reviewing a governed scope, load every matching installed coding-style skill. Go files, go.mod, or go.sum => go-coding-style; non-Go scopes => every matching installed style skill. Resolve the repository formatter/linter/config and exact source anchors, then check actual adherence. Missing or unverified admission blocks. A cosmetic-only issue is a NIT; material style/quality rules are findings. The style critic reports findings only; the implementer fixes them. A style label never downgrades behavior, security, interface, test/proof, architecture, or ownership failures; Critic B or Critic C reports those hard consequences.

#### Critic B — correctness/fidelity

Critic B is ordinary and non-authoritative. It guards behavior, security, interface, test/proof, architecture, ownership, and other hard consequence contracts; none may be downgraded as style.

#### Critic C — long-term health

Critic C is a fresh special `sol-high` reviewer. It owns skip-design code admission and final reconciliation, and it never edits.
...
| **Execution Reviewer** | Three critic lenses per review | 3 | Critic A coding style is ordinary and reports only; Critic B correctness/fidelity is ordinary and guards hard contracts; **Critic C long-term health is special** and owns skip-design code admission and all code reconciliation. Review targets: root aggregate, sub-task, candidate fix. Root scope blocks. Add E2E where code applies. Report only; no critic edits. |
...
One execution lane per independent unit. Spawn fresh blind Critic A and Critic B identities for each review, and spawn a fresh special Critic C long-term-health reviewer for each review invocation; retire prior special slots without shutdown or terminal cleanup. Root aggregate review starts after root E2E/proof and uses all three critics, plus E2E where code applies.
...
**Manual skill refresh (coordinator, lead, snitch).** Lead uses `followup_task` for an idle role or `send_message` for a running role to request re-invocation after context compaction, phase changes, long waits, and user-waiting resume. When a nested ECI is active, the coordinator and lead must also re-read the entire `skills/explore-critique-implement/SKILL.md` and re-invoke its instructions immediately after the authoritative `PostCompact` compaction refresh signal and before any next decision or tool call. `PostCompact` is the authoritative compaction refresh signal. `SessionStart` `startup|resume|clear` is only a best-effort resume/clear reminder, not proof that compaction occurred. Silence is non-terminal; do not create a wait-retry loop.
...
| submitted -> in_review | Coordinator verifies submission checklist and current admission pass. Code targets: route the full root-task diff to fresh blind Critic A coding style, Critic B correctness/fidelity, and Critic C long-term health; add E2E when code applies. Non-code targets: route to a verifier unless a role-specific paired reviewer applies. |
| in_review → in_progress | Root REJECTED/CONDITIONAL. Create/fix tasks. REJECTED reruns all three critics after proof/amend; CONDITIONAL fixes verify before final proof/QA |
...
| Review | Fresh blind Critic A coding-style, Critic B correctness/fidelity, and Critic C long-term-health lenses inspect the integrated code diff in parallel; add E2E where code applies. |
| Repair | REJECTED reruns all three critics after fix/proof/amend. CONDITIONAL creates required pre-QA fix tasks. |
...
6. **Requirement traceability** -- map every component and executable lane to active canonical requirement refs, immutable lane/predecessor records, complete paths/edges, and the full requirement → decision/reasoning → parent/derived lane → executed assignment chain. Cover every user requirement and justify every component/lane.
...
| Executor | Execution Reviewers | Sub-task/candidate-fix ready | Direct, async. Assign fresh blind Critic A coding-style and Critic B correctness/fidelity identities plus a fresh special Critic C long-term-health identity; add E2E where code applies. Executor continues in-flight work. |
...
- Candidate-fix reviews use sub-task/candidate-fix execution review rules: all three critics, async verdicts, independent verification, and **Impact-proportional pre-routing**. Async output never delays or reopens root aggregate review except for verified admission invalidation under **Coding-style admission**.
...
**Execution three-critic review:** Spawn fresh blind Critic A coding style and Critic B correctness/fidelity identities in parallel with a fresh special Critic C long-term-health identity; add E2E when code applies. Review independently first and withhold the verdict until all required reports arrive. Critic A loads every matching installed coding-style skill, including `go-coding-style` for Go files, `go.mod`, or `go.sum`, and reports only; Critic B guards every hard consequence contract; Critic C independently admits skip-design code before writing and reconciles actual changed scope, admissions, approved deltas/deviations, and post-write Tool evidence. Critic C judges final state only: no change-history defense; artifact must stand on its own. Execution gate verdicts apply only to `now` original-scope or mixed-required portions: REJECTED for Critical or Foundational; CONDITIONAL for Major; APPROVED when no blocking original-scope finding remains. APPROVED may include NIT notes. Missing/unverified admission or verified invalidation is a prerequisite gate, not a CONDITIONAL authorization or async finding to queue. Pre-route all findings: root aggregate REJECTED/CONDITIONAL and required pre-QA work remain `now`; only deadline-qualified defer or scope-creep debt enters `queued_async_followup`; `ignored-contradictory` directives record only. Scope-creep debt queues under its scope-screen record and requires the exact source comment with its specific tracker reference when an affected source exists; otherwise require the specific tracker record. When all three critics cover an in-scope potential defer, all three independently verify the same terminal `sealed-at <= started + 2 minutes`; they cannot reopen its clock. Root aggregate REJECTED reruns all three critics after fixes, proof, and amend/squash. Root aggregate CONDITIONAL creates required pre-QA fix tasks; fix and verify before final proof/QA. Async output never delays, reopens, or retroactively blocks root aggregate review except for verified admission invalidation under **Coding-style admission**.
...
- [ ] Critic A coding style: load every matching installed style skill; Go files, go.mod, or go.sum => go-coding-style; resolve formatter/linter/config anchors; report material findings only and leave edits to the implementer; cosmetic-only issues are NITs.
- [ ] Critic B correctness/fidelity: classify by consequence; false behavior/name/interface claims, security, RCA, testing/proof/TDD, and approved architecture/file-ownership/purpose/interface contracts remain hard failures and cannot be downgraded as style.
- [ ] Critic C long-term health: load every matching installed style skill and reconcile actual scope, admitted record, approved deltas/deviations, and post-write Tool evidence. Invocation alone is not compliance. Use the fresh special `sol-high` route and diff-only intention packet where applicable.
...
- [ ] Claim scope — compare mechanism/predicate, emitted wording, supported wording, and boundary counterexample/negative test; silent `UserPromptSubmit` state maintenance is not described as reminder emission or an LLM reviewer/classifier; the optional LLM first-tool review is manual or asynchronous/cached-only and is deliberately absent from the synchronous `PreToolUse` chain.
...
**AGGREGATE REVIEW INVARIANT:** Async execution reviews run during execution per the three-critic review gate. Start root aggregate review after known slices/sub-tasks land, root E2E/proof passes, and one commit per touched repo exists. Independently verify, then pre-route arrived async output; do not wait for pending output. `now` work follows normal routing; only deadline-qualified defer or scope-creep debt enters `queued_async_followup`; `ignored-contradictory` directives record without reopening. Verified coding-style admission invalidation instead follows **Coding-style admission**, pauses affected work, and cannot queue past root review or QA. Root REJECTED findings become tasks; fix, re-prove, amend/squash, rerun all three critics. Root CONDITIONAL findings become required pre-QA fix tasks; fix and verify before final proof/QA. After no root REJECTED/CONDITIONAL remains, rerun full E2E/proof before QA.
...
3. **Tasks with dependencies first**, then request lead to spawn teammates to claim them. Every task description and lane assignment includes claim tagging, the structured lane/assignment fields, canonical `requirement_refs`, `derivation_reason`, complete paths/edges, the expanded requirement chain, its governed coding-style scope, admission role, and admitted record or read-only proposal duty when applicable.
...
| Coordinator assigns new task to executor | Verify file ownership, dependencies, root-task grouping, and sub-task/candidate-fix review trigger with Critic A coding-style + Critic B correctness/fidelity + Critic C long-term-health + E2E where code applies |
...
- [ ] Lineage admission passed before the provider call: active-admission refs resolve uniquely to active requirements; historical/provenance refs carry immutable `state_event_ref` and never authorize work; root/child/reroute/aggregate predecessor rules, same-root ancestry, acyclicity, ordered complete paths, derivation necessity, and evidence hashes hold; the chain reaches a user requirement
...
| Async execution review lacks the three critics | STOP. Spawn fresh blind Critic A coding style and Critic B correctness/fidelity plus a fresh special Critic C long-term-health lens, with E2E where code applies. |
...
| Root aggregate reviewer feedback ignored | Coordinator enforces REJECTED fixes and required pre-QA fix tasks. Async scope follows the three-critic review gate. |
...
| False behavior/name/interface, security, RCA, testing/proof/TDD, or approved architecture/ownership/purpose/interface result labeled a style deviation | STOP. Critic B or Critic C applies the corresponding hard verdict. |
...
| A critic omits matching style skills or the admitted record | STOP. Critic A loads every exact match; Critic B guards hard contracts; Critic C reconciles admission/deltas/Tool evidence. |
...
| Fewer than three execution-review critics active for a target | Spawn the missing fresh blind Critic A/B/C identity; add E2E when code applies. |
