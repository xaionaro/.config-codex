...
- **Document every function declaration and method, each type or constant declared at any scope, and each package-level variable, including unexported declarations.** Give each a concise, clear comment immediately before its declaration. State its semantics and most salient generic or common use case; if none exists, state its primary or intended use.
